...
- **`OnnxExecutionProvider` enum trimmed** to `Cpu`, `Cuda`, `CoreMl`. Removed variants: `DirectMl`, `Rocm`, `OpenVino`, `Qnn`, `TensorRt`, `WebGpu`.
- **`execution_providers` strings**: `"directml"`, `"rocm"`, `"openvino"`, `"qnn"`, `"tensorrt"` / `"nvrtx"`, `"webgpu"` / `"wgpu"` are no longer recognized at the catalog layer.
