...
pub(super) struct LoadedOnnxSession {
...
/// Load (or fetch from cache) the raw ONNX runner for `spec`.
///
...
pub(super) async fn load_raw(
    ...
    let path = resolve_model_path(base_dir, spec, &options).await?;
