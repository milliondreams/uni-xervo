...
impl ModelProvider for MockProvider {
    ...
    async fn load(&self, spec: &ModelAliasSpec) -> Result<LoadedModelHandle> {
        ...
        match spec.task {
            ...
            // ModelTask is `#[non_exhaustive]` — required wildcard for
            // matches outside the defining crate. The earlier
            // `supported_tasks.contains` check rules out any unknown
            // variant from reaching this point.
...
pub async fn runtime_with_image_embedder() -> Result<Arc<ModelRuntime>> {
    let provider = MockProvider::image_embed_only();
    let spec = make_spec(
        "embed_image/test",
        ModelTask::EmbedImage,
        "mock/image-embed",
...
pub async fn runtime_with_audio_embedder() -> Result<Arc<ModelRuntime>> {
    let provider = MockProvider::audio_embed_only();
    let spec = make_spec(
        "embed_audio/test",
        ModelTask::EmbedAudio,
        "mock/audio-embed",
...
pub async fn runtime_with_multimodal_embedder() -> Result<Arc<ModelRuntime>> {
    let provider = MockProvider::multimodal_embed_only();
    let spec = make_spec(
...
pub async fn runtime_with_nlp_model() -> Result<Arc<ModelRuntime>> {
    let provider = MockProvider::nlp_only();
    let spec = make_spec("nlp/test", ModelTask::Nlp, "mock/nlp", "test-model");
    ModelRuntime::builder()
        .register_provider(provider)
        .catalog(vec![spec])
        .build()
...
pub async fn runtime_with_document_extractor() -> Result<Arc<ModelRuntime>> {
    let provider = MockProvider::document_extract_only();
    let spec = make_spec(
        "document_extract/test",
        ModelTask::DocumentExtract,
        "mock/doc-extract",
        "test-model",
    );
...
pub async fn runtime_with_transcriber() -> Result<Arc<ModelRuntime>> {
    let provider = MockProvider::transcribe_only();
    let spec = make_spec(
        "transcribe/test",
        ModelTask::Transcribe,
        "mock/transcribe",
...
pub async fn runtime_with_ocr_model() -> Result<Arc<ModelRuntime>> {
    let provider = MockProvider::ocr_only();
    let spec = make_spec("ocr/test", ModelTask::Ocr, "mock/ocr", "test-model");
    ModelRuntime::builder()
        .register_provider(provider)
        .catalog(vec![spec])
        .build()
